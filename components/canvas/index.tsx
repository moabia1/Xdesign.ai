...
import CanvasFloatingToolBar from "./floating-toolbars";
...
const Canvas = ({
  ...
  const { theme, frames, loadingStatus, selectedFrame, setSelectedFrameId } =
    useCanvas();
  ...
  const saveThumbnailToProject = useCallback(async (
  ...
  const handleCanvasScreenshot = useCallback(async () => {
    try {
      ...
      const title = projectName || "canvas-screenshot";
  ...
  }, [setSelectedFrameId, projectName]);
...
function CanvasLoader({ status }: { status?: LoadingStatusType | "fetching"  | "finalizing" }) {
  return (
    <div
      className={cn(
        `absolute top-4 left-1/2 -translate-x-1/2 min-w-40 max-w-full px-4 pt-1.5 pb-2 rounded-br-xl rounded-bl-xl shadow-md flex items-center spaxe-x-2 z-10`,
